...
app.controller("appController", function($scope, $sce) {
    ...
    requestForWeatherData = () => {
        ...
        fetch(`api.openweathermap.org/data/2.5/weather?id=${cityId}&appid=${config.apiKey}&units=${$scope.userConfig.units}`)
        .then( (response) => {
            return response.json();
        }).then( (data) => {
            const dataObj = {
            ...
            $scope.$apply();
            // 5 day forecast
            fetch(`api.openweathermap.org/data/2.5/forecast?id=${cityId}&appid=${config.apiKey}&units=${$scope.userConfig.units}`)
            .then( (response) => {
                return response.json();
            }).then( (data) => {
